 ...
 * 知识问答分页Sitemap生成器
 *
 * 动态生成知识问答页面的Sitemap，支持无限分页
 * 如果API失败，返回包含首页的空Sitemap
 */

export const GET: APIRoute = async () => {
  console.log('[Sitemap] 开始生成知识问答分页Sitemap...');

  try {
    // 获取所有知识问答数据（自动分页）
    const knowledgeItems = await SitemapGenerator.fetchAllPaginatedData('knowledge', 'published');

    // 生成URL列表
    const items = knowledgeItems.map(item => ({
      url: SitemapGenerator.generateUrlTemplate('knowledge', item),
      ...
      priority: SitemapGenerator.calculatePriority('knowledge', item.updated_at || item.created_at)
    }));
    ...
    console.log(`[Sitemap] 知识问答Sitemap生成完成，共 ${items.length} 个URL`);
    ...
    console.error('[Sitemap] 知识问答Sitemap生成失败:', error);
