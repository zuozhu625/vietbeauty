...
本文档详细说明越南医疗整形项目的完整技术架构，采用**双数据库架构**，集成了**三大智能体系统**（DQA、翻译智能体、质量检查智能体），包括系统层次、技术选型、数据流向、部署架构等，帮助开发者快速理解项目的整体设计。
...
本项目采用**三大智能体协同工作**的架构，实现医美知识库的自动化生成、翻译和质量保证：
...
| 特性 | DQA系统 | 翻译智能体 | 质量检查智能体 |
|------|--------|----------|----------------|
| **版本** | v1.5.0 | v3.0 | v1.0 |
| **脚本文件** | `backend/src/dqa/dqaGenerator.js` | `scripts/qa_translator_agent.py` | `scripts/qa_quality_checker.py` |
| **主要功能** | 生成医院问答 | 翻译中文为越南语 | 检查和修正质量问题 |
| **数据源** | 122家医院 | medical.db (1,829条) | database.sqlite |
| **输出语言** | 越南语 | 越南语 (100%纯) | 越南语 (修正) |
| **运行频率** | 每15分钟1条 | 每15分钟1条 | 手动触发/定期检查 |
| **日产量** | 96条/天 | 96条/天 | 按需检查 |
| **使用技术** | 模板+随机 | LLM翻译 | LLM检查修正 |
| **质量保证** | 拟人化表达 | 均匀分布 | 自动修正问题 |

### 1. DQA系统 (v1.5.0)

**技术栈**: JavaScript + Node.js + SQLite  
**文件位置**: `/root/越南医疗整形项目/backend/src/dqa/`

**核心特性**:
- ✅ 基于122家医院真实数据
- ✅ 6种问答类型 (资质、等级、服务、地址、联系、医生)
- ✅ 15种开场白 + 10种结尾语 + 14种过渡词
- ✅ 医院咨询专属分类
- ✅ 15分钟自动生成1条问答
...
```
backend/src/dqa/
├── dqaGenerator.js        # 问答生成器
├── dqaService.js          # 服务主控制器
├── dqaScheduler.js        # 定时任务调度
├── dqaRoutes.js           # API路由
├── hospitalDataExtractor.js # 医院数据提取
├── chainHospitalEnhancer.js # 连锁医院增强
└── README.md              # 技术文档
```
...
**技术栈**: Python + Doubao LLM API + Ark API  
**文件位置**: `/root/越南医疗整形项目/scripts/qa_translator_agent.py`

**核心特性**:
- ✅ 1,829条中文问答翻译
- ✅ 5大分类均匀分布 (丰胸、五官、产修、吸脂、牙矫)
- ✅ 100%纯越南语保证 (中文检测+自动清理)
- ✅ 5次重试机制
- ✅ 智能循环，覆盖全部问答
...
{
    "丰胸": "Thẩm mỹ ngực",
    "五官": "Thẩm mỹ khuôn mặt", 
    "产修": "Phục hồi sau sinh",
    "吸脂": "Hút mỡ tạo dáng",
    "牙矫": "Niềng răng"
}
```
...
**技术栈**: Python + Doubao LLM API  
**文件位置**: `/root/越南医疗整形项目/scripts/qa_quality_checker.py`
...
# 检查特定分类
python scripts/qa_quality_checker.py --category "Thẩm mỹ ngực" --batch 50
